...
| The report | `application/ExceptionReportServiceTest` (extended) | U | **FR-019 [review]**: a released batch is `BATCH_RELEASED` and not `BATCH_FAILED`; the switch over kinds stays exhaustive; the log event carries the new kind. **The CLI half is owed by the 005 tree** (amended 2026-09-21): `batch/cli/ReportExceptionsCliTest` is that tree's file under the coordination contract, and 004 touched it only for the counts-line literal a sixth kind forces (recorded as a hand-off at T036). The case that asserts a `BATCH_RELEASED` entry appears in the printed table and in the CSV is 005's to land; the kind travels by name, so nothing is broken without it. |
