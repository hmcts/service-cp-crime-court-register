...
`LogEventReportSink` writes one `courtregister_exception_report` summary event - **eleven fields:
the run id, the three instants, the five counts and what the entry cap dropped, and no delivery
status, because a sink cannot observe its own delivery or the other sink's** - and one `courtregister_exception` event per
...
| `courtregister.report.max-entries` | `5000` | How many exceptions one report may carry. Over it the **oldest** are kept and the rest are counted as `truncated` on the summary event and on both run lines; the five counts are **not** capped, because a count that shrank with the list would make the worst morning of the year read as a quieter one. A zero is refused at startup: a report that carries nothing is indistinguishable from a morning with nothing wrong on it |
...
└── config/
    ...
    ├── ReportEmailConfig.java             # NEW: behind courtregister.report.email.enabled alone -
    ...
    │                                       #   courtregister.generation.fileservice-mode is unchanged
