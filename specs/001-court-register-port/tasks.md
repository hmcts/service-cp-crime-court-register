...
- [x] T002 Vendor the frozen outbound contract: `progression.add-court-register.json` +
      ...
      `src/test/resources/contracts/progression/` with `PROVENANCE.md` (tag, jar digest,
      progression `pom.xml:75` coordinate). *(done — commit `e83693c`)*
...
- [ ] T010 [P] [US4] Write `config/ProcessingMetricsTest` (red; seam: `ProcessingMetrics`) — one
      case per instrument incl. `courtregister_completions_total{reason}` for the five reasons and
      `courtregister_deadlettered_total{reason}`.
- [ ] T011 [P] [US1] Write `persistence/SchemaMigrationIT` (red) — every data-model V1 fact,
      ...
      `response_code`, status CHECK PENDING/POSTED/FAILED, FK ON DELETE RESTRICT.

### Implementation

- [ ] T012 [US1] Implement `domain/DistributionCommand`, `inbound/DistributionCommandParser`,
      `config/JacksonConfig`, `src/main/resources/contracts/distribution-command.schema.json`
      (green for T007).
...
- [ ] T023 [P] [US3] Write `pipeline/DatesTest` (red; seam: `Dates`) — DT1–DT8 twins minus DT5;
      **C10 fix**: `registerDate('2020-06-01T10:00:00Z')` is the same instant (fails vs legacy
      ...
      `Invalid date format` case; **C12**: BST-boundary hearing (23:30 BST) resolves the refdata
      day from the true instant.
...
- [ ] T028 [P] [US3] Write `pipeline/SubscriptionRulesTest` (red; seam: `SubscriptionRules`) —
      ...
      `matchCpsProsecuted` not reproduced.
...
- [ ] T034 [US3] Implement `pipeline/VocabularyBuilder` (green T025) — updates C30.
...
- [ ] T037 [US3] Implement `pipeline/SubscriptionRules` (green T028) — updates C4/C5.
...
### Tests first ⚠️ (all [P] — one file each)
...
- [ ] T045 [P] [US3] `pipeline/OffenceMapperTest` — OF1 twin repaired (real indicatedPlea,
      ...
      verdictType.verdictCode` (`"1234"`, fails vs legacy `"desc1234"`); **C24 fix**: wording
...
- [ ] T047 [P] [US3] `pipeline/ProsecutionCaseOrApplicationMapperTest` — PC1–PC7 twins (correct
      ...
      application and personDefendant-less own-record are guarded skips with WARN + reason (fail
      vs legacy TypeError), C22 exhibit re-asserted at mapper level, dead methods not reproduced
      (C26).
- [ ] T048 [P] [US3] `pipeline/RecipientMapperTest` — R1–R4 twins + `cr_standard` default pinned;
      **C27 fix**: letter-delivery and missing-email drops logged + counted (drop preserved);
      emailAddress2 carried.
...
- [ ] T050 [P] [US3] `pipeline/YouthDefendantMapperTest` — YD1 twin repaired (real nationality,
      ...
      with reason (fails vs legacy TypeError); **C25 fix**: ethnicity observed-else-self-defined
...
- [ ] T056 [US3] Implement `pipeline/RegisterTransformationChain` + `pipeline/AggregationMapper`
      wiring into `RegisterTransformer` (green T051 chain cases; `DistributionPipelineTest`
      end-to-end unit path now green).
...
- [ ] T068 [US4] `config/PipelineConfig` + final `application.yaml` + `HttpSurfaceTest`,
      `SharedObjectMapperTest`, `TelemetryPrivacyTest` (privacy test written first, red on a
      seeded violation, then the logging swept) — actuator-only surface; no PII at INFO+.
...
- **Only test tasks are ever [P].** Implementation tasks touching shared production files execute
  ...
  T054 → T055 → T056 (document assembly), T065–T067 (adapter configs), T068 (final wiring).
