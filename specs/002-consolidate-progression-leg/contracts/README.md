...
| `fileservice/` | `file-service-liquibase-db-changelog.xml` + changesets `001`–`006` (`metadata(file_id, metadata jsonb)`, `content(file_id, content bytea, deleted, deleted_at)`) | `framework-libraries` `58aad8664` (**2023-12-22**, the local clone is old; **verify against a running STE or SIT `fileservice` database before T033 starts** (T005); both candidate servers were stopped for the evening of 2026-09-05). **2026-09-05: progression's resolved dependency is file-service 17.103.1 (target/dependency jars); its persistence jar issues exactly `INSERT INTO metadata(metadata, file_id) values (to_json(?::json), ?)` and `INSERT INTO content(file_id, content, deleted) VALUES(?, ?, ?)` and updates `content.deleted_at`, consistent with changesets 001-006; the live-table check (\d metadata / \d content) could not be run because the STE-42 Postgres and the SIT psf-sit-ccm01-system server were stopped for the evening - still to do against a running stack.** Path: `file-service/file-service-liquibase/src/main/resources/liquibase/` | `FileServicePayloadStoreIT` (Testcontainers Postgres seeded with these changesets) |
