...
**Merge subjects are outside that list, and this is a statement of practice rather than a TDD
exception.** The list governs the subject an author composes for a change they wrote; a merge
commit's subject is generated by the tool that performs the merge, out of the refs being joined,
and the merge itself is not a change to this codebase. Read out of `git log --merges` rather than
counted from memory: seven merge commits are reachable from this branch and **six of them are
`main`'s own pull-request merges** - `Merge pull request #2`, `#3`, `#5`, `#8`, `#9` and `#10`,
every one a dependency update raised and merged on `main`, two of them (`c9d7be0`, `0d878ed`) on
`main` before this branch was cut and four (`653bd3f`, `6d47a47`, `d76a8a7`, `ae4a6e9`) arriving
with the merge at `41009d0`. None of the six was ever this branch's to write, and none can be
rewritten now: they are published on `main`. The seventh, `41009d0` itself, is this branch's own
`Merge remote-tracking branch 'origin/main' into 002-consolidate-progression-leg`, which is the
subject git composes for that operation; why that merge happened is in the Phase 8 checkpoint.
No merge on this branch carries content of its own, so there is nothing under a merge subject that
an accepted type would have described better.
...
**Checkpoint**: review gate 8, **and the gate's first-round findings are closed in the thirteen
...
on T072's line above. **The gate then ran a second round, whose five findings are closed in the nine
commits `41009d0..HEAD`** and whose numbering is reconciled against the first round's below.
...
second-round finding 5 is the merge, recorded above. Nine commits close the round,
`41009d0..HEAD`.
...
block and this is where one would have gone. What puts each of the nine commits of
`41009d0..HEAD` outside it:
