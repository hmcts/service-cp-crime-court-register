...
| `notificationnotify-send-email-notification.json` | `POST /notificationnotify-command-api/command/api/rest/notificationnotify/send-email-notification`, `Content-Type: application/vnd.notificationnotify.send-email-notification+json` → **202**, with the three fields `notificationnotify.command.send-email-notification.json` marks required. |
