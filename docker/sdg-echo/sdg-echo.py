...
def document_available_from(generate_document):
    """The event the real systemdocgenerator would publish for this generate-document command."""
    return {
...
def publish(event):
    ...
    with socket.create_connection((ARTEMIS_HOST, ARTEMIS_PORT), timeout=10) as broker:
        ...
        broker.sendall(frame("SEND", {
            ...
            # to a topic the whole estate publishes to ever sees this message.
