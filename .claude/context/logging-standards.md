...
>   remains forbidden, at any level in a deployed environment for whole payloads — and every
>   defendant on this register is a youth, which raises the stakes rather than changing the rule.
> - There is **no REST API**, so there are no request/response or `@ControllerAdvice` log points to
>   standardise — the units of work are queue messages and one per-hearing submission.
