...
| The frozen outbound contract | the vendored copies under `src/main/resources/contracts/progression/` (source: `cpp-platform-core-domain` tag `v17.103.13`, `criminal-court-public-model` `courtRegisterDocument/*`; endpoint RAML in `cpp-context-progression` `progression_command_api.raml`) |
...
- Distinguish facts from inferences — "the JS does X" must be a quoted line, never a recollection; legacy is the behavioural oracle for everything not catalogued in `doc/DEFECT-FIXES.md`
- Flag uncertainty explicitly
- Present options with trade-offs, not single recommendations
- When a legacy behaviour looks like a bug, check `doc/DEFECT-FIXES.md`: if it is one of the 34 catalogued defects, the fix specification there governs; if it is NOT catalogued, say so and recommend registering it before anyone changes it — never a silent fix, in either direction
